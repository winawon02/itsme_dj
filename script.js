...
(() => {
  const grid = document.querySelector('#itsme-content.page-branch-native .branch-native-event-grid');
  if (!grid) return;

  fetch('/event', {credentials: 'same-origin'})
    .then(response => {
      if (!response.ok) throw new Error(`Event board returned ${response.status}`);
      return response.text();
    })
    .then(html => {
      const page = new DOMParser().parseFromString(html, 'text/html');
      const board = page.querySelector('#w20260921b5ec962484e96 .widget.board');
      if (!board) throw new Error('Event board was absent from the response');

      const posts = [...board.querySelectorAll('._card_wrap')].map((card, order) => {
        const sourceLink = card.querySelector('a.post_link_wrap[href*="bmode=view"]');
        const title = card.querySelector('.title-block');
        const image = card.querySelector('.card-thumbnail-wrap');
        const date = card.querySelector('small.date')?.getAttribute('title') || '';
        if (!sourceLink || !title) return null;
        const url = new URL(sourceLink.getAttribute('href'), location.origin);
        if (url.origin !== location.origin || url.pathname.replace(/\/$/, '') !== '/event') return null;
        return {
      ...
      if (!posts.length) {
        grid.innerHTML = '<p class="branch-native-event-status">현재 진행 중인 이벤트가 없습니다.</p>';
        return;
      }
      const cards = posts.map(post => {
        ...
        if (post.category) {
          const category = document.createElement('span');
          category.className = 'branch-native-event-category';
